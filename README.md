...
Skill 会在动手前先问七个问题，因为它们决定整轮的产出形态。
选项用**字母**标注（数字留给输入文本的行号）。可直接回「全默认」。

| Gate | 问什么 | 选项 |
|---|---|---|
| **A 处理模式** | 做什么 | 只优化中文 / 翻译成日语 / 只做 TTS 读音纠正 / 翻译+TTS |
| **B 中文译名偏好** | 用哪套译名 | 贴近官方（爽世）/ 更社区化（素世） |
| **C 成稿用途** | 给谁看 | 配音用 / 字幕用 / 两者都要 |
| **D 敬语档位** | 语体严格度 | 按原作设定 / 本场景放宽 / 全篇统一 |
| **E 改动幅度** | 我下手多重 | 只改错误 / 允许口语化改写 / 允许重构句子 |
| **F 批次一致性** | 多段时 | 逐句独立 / 整批次一致 |
| **G 保留层**（仅模式 D） | 做哪些层 | 全层 / 跳过断句 / 跳过读音 |

默认值：`A-B / B-A / C-C / D-A / E-B / F-B / G-A`。
每次输出开头会回显实际采用的档位，便于你发现哪项理解错了。

**几个关键约束**：

- 模式**只做 TTS 读音纠正**时**不做角色化**——已有日语是正确台词，只处理读法。
...
模式 A 需要 `characters/<角色>.zh.md`（中文层规则），**这些文件尚未建立**。
